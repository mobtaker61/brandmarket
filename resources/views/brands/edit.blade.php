...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">ویرایش برند</h1>
                <p class="text-gray-600">{{ $brand->name }} - {{ $brand->company_name }}</p>
            </div>
            <div class="flex space-x-2 space-x-reverse">
                <a href="{{ route('brands.show', $brand) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
    ...
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('brands.update', $brand) }}" method="POST" enctype="multipart/form-data" x-data="brandForm()">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" {{ old('country_id', $brand->country_id) == $country->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                        >
                            <option value="official" {{ old('iran_market_presence', $brand->iran_market_presence) == 'official' ? 'selected' : '' }}>رسمی</option>
                            <option value="unofficial" {{ old('iran_market_presence', $brand->iran_market_presence) == 'unofficial' ? 'selected' : '' }}>غیررسمی</option>
                            <option value="absent" {{ old('iran_market_presence', $brand->iran_market_presence) == 'absent' ? 'selected' : '' }}>غایب</option>
                        </select>
                        ...
                        @foreach($categories as $category)
                            <div class="flex items-center">
                                <input
                                    ...
                                    name="category_ids[]"
                                    value="{{ $category->id }}"
                                    {{ in_array($category->id, old('category_ids', $brand->categories->pluck('id')->toArray())) ? 'checked' : '' }}
                                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                >
                                <label for="category_{{ $category->id }}" class="mr-2 text-sm text-gray-700">
                                    {{ $category->icon ?? '' }} {{ $category->name }}
                                    @if($brand->categories->where('id', $category->id)->first()?->pivot->is_primary)
                                        <span class="text-xs bg-blue-200 px-1 rounded mr-1">اصلی</span>
                                    @endif
                                </label>
                            </div>
                        @endforeach
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="instagram"
                            name="instagram"
                            value="{{ old('instagram', $brand->instagram) }}"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="telegram"
                            name="telegram"
                            value="{{ old('telegram', $brand->telegram) }}"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="linkedin"
                            name="linkedin"
                            value="{{ old('linkedin', $brand->linkedin) }}"
                    ...
                    <div>
                        ...
                        >
                        @if($brand->logo)
                            <img src="{{ $brand->logo }}" class="h-16 mt-2 rounded border object-contain" alt="پیش‌نمایش لوگو فعلی">
                        @endif
                        <template x-if="$refs.logo_file && $refs.logo_file.files && $refs.logo_file.files[0]">
                            <img :src="URL.createObjectURL($refs.logo_file.files[0])" class="h-16 mt-2 rounded border object-contain" alt="پیش‌نمایش لوگو جدید">
                        </template>
                        @error('logo')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    >{{ old('notes', $brand->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-2 space-x-reverse mt-8">
                    <a href="{{ route('brands.show', $brand) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg font-medium transition-colors">
                        انصراف
                    </a>
